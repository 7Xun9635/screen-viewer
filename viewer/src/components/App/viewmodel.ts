...
export class AppViewModel {
  ...
  // Refresh button visibility: shown on mouse movement, auto-hidden after 2s of inactivity.
  @observable
  private refreshVisible = false;

  private refreshHideTimer: NodeJS.Timeout | undefined;
  ...
  // True when the page was opened with a ?room= link: jump straight to the view page.
  @computed
  public get hasRoom() {
    return !!this.room;
  }
  ...
  @computed
  public get isRefreshVisible() {
    return this.refreshVisible;
  }

  // Send a join request to the signaller (room + password only).
  // No approval step: the sharer auto-admits anyone holding the invite link.
  public sendJoin = () => {
    if (!this.room) return;
    this.serverConnection.send(
      JSON.stringify({
        ...
        room: this.room,
        from: this.uuid,
        name: this.name || "viewer",
        auth: {"type": "password", "password": this.password}
      })
    );
  };

  // Refresh the view to recover from a frozen frame caused by network jitter.
  // Reloading re-runs the auto-join instantly, re-establishing the stream.
  public refresh = () => {
    this.terminate();
    window.location.reload();
  };

  // Show the refresh button on mouse movement; auto-hide 2s after the last move.
  public onMouseMove = () => {
    if (!this.refreshVisible) {
      this.refreshVisible = true;
    }
    if (this.refreshHideTimer) clearTimeout(this.refreshHideTimer);
    this.refreshHideTimer = setTimeout(() => {
  ...
  constructor() {
    ...
    this.serverConnection.onopen = () => {
      ...
      // Auto-join immediately when opened with a room link: no join page, no approval.
      if (this.room) this.sendJoin();
    };
    ...
    // Show the refresh button on mouse movement; auto-hide after 2s of inactivity.
